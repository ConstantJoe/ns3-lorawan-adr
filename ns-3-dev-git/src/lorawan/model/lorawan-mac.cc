...
LoRaWANMac::sendTRXStateRequestForIdleMAC ()
{
  if (m_deviceType == LORAWAN_DT_END_DEVICE_CLASS_A) {
    //NS_LOG_INFO ("set phy off on class a");
...
void
LoRaWANMac::PdDataIndication (uint32_t phyPayloadLength, Ptr<Packet> p, uint8_t lqi, uint8_t channelIndex, uint8_t dataRateIndex, uint8_t codeRate, double sinrAvg)
{
  // TODO: which state?
  //
  if (m_deviceType == LORAWAN_DT_END_DEVICE_CLASS_A) {
...
LoRaWANMac::sendMACPayloadRequest (LoRaWANDataRequestParams params, Ptr<Packet> p)
{
  ...
  NS_ASSERT_MSG(params.m_loraWANDataRateIndex != 6, "in sendMACPayloadRequest");
...
LoRaWANMac::ConfigurePhyForTX () {
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO ("configure phy for tx");
  ...
  if (m_txPkt != 0) {
    ...
    uint8_t txPowerIndex = txQElement->lorawanDataRequestParams.m_loraWANTxPowerIndex;
    ...
    uint8_t maxTxPower = m_lorawanMacRDC->GetMaxPowerForSubBand (subBandIndex) - (2 * txPowerIndex); // note txPowerIndex will always be 0 for Gateway transmissions, possibly not for EDs (set in ADR). TODO: this is only the process for Europe, support other regions

    NS_ASSERT_MSG(dataRateIndex != 6, " DR6 not supported yet. 1.");

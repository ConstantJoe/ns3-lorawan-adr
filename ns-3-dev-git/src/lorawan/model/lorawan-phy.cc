    ...
    : m_setTRXState (), m_index (index)
{
  ...
  m_txPsd = psdHelper.CreateTxPowerSpectralDensity (m_txPower,
                                                    freq);
  m_noise = psdHelper.CreateNoisePowerSpectralDensity (freq); //TODO: does this make sense? Only noise at the centre frequency would be modeled, but the device can receive from different channels.
...
LoRaWANPhy::DoDispose (void)
{
  ...
  m_signal = 0;
  m_errorModel = 0;
  m_pdDataIndicationCallback = MakeNullCallback< void, uint32_t, Ptr<Packet>, uint8_t, uint8_t, uint8_t, uint8_t , double> ();
...
{
  NS_LOG_FUNCTION (this << power << " " << channelIndex << " " << dataRateIndex << " " << codeRate << " " << preambleLength << " " << implicitHeader << " " << crcOn);

  PrintCurrentTxConf();

  if(dataRateIndex == 6) {
    NS_LOG_INFO("data rate is 6!");
  }
  ...
  // possible tx power values: 0, -2, -4, -6, -8, -10, -12, -14 (relative to max EIRP)
  // max EIRP is 27 for high power band, otherwise 14dBm
  // so possible tx power values are those listed below
  //TODO: but because in other regions the tx pow indexes modify the pow by 3 each index, this should really be done relatively based on the deployed area
  if (!(power == 27 || power == 25 || power == 23 || power == 21 || power == 19 || power == 17 || power == 15 || power == 13 
     || power == 14 || power == 12 || power == 10 || power ==  8 || power ==  6 || power ==  4 || power == 2  || power ==  0 ))
    validConf = false;
  ...
  if (m_trxState == LORAWAN_PHY_RX_ON && !m_setTRXState.IsRunning ())
    {
      ...
      //NS_LOG_INFO("interference 1:" << *interferenceAndNoise);
      //NS_LOG_INFO("rx psd:" << *loraWanRxParams->psd);
      *interferenceAndNoise -= *loraWanRxParams->psd;
      //NS_LOG_INFO("interference 2:" << *interferenceAndNoise);
      *interferenceAndNoise += *m_noise;
      //NS_LOG_INFO("interference 3:" << *interferenceAndNoise);

      //NS_LOG_INFO("NOISE value:" << *m_noise);
      ...
      NS_LOG_INFO("sinr calc values: psd: " << *(loraWanRxParams->psd) << " freq: " << freq << " TotalAvgPower: " << LoRaWANSpectrumValueHelper::TotalAvgPower (loraWanRxParams->psd, freq)
        << " interferenceAndNoise: " << *interferenceAndNoise << " TotalAvgPower: " << LoRaWANSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, freq) << " divided: " << 
        LoRaWANSpectrumValueHelper::TotalAvgPower (loraWanRxParams->psd, freq) / LoRaWANSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, freq) << " log10 of that: " <<
        log10 (LoRaWANSpectrumValueHelper::TotalAvgPower (loraWanRxParams->psd, freq) / LoRaWANSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, freq)) << " *10: " <<
        10.0 * log10 (LoRaWANSpectrumValueHelper::TotalAvgPower (loraWanRxParams->psd, freq) / LoRaWANSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, freq))
        );
      ...
      NS_LOG_INFO("sinr_db " << sinr_db);
      NS_LOG_INFO("sinr_cutoff_db" << sinr_cutoff_db);
      //TODO:
      //the only noise in the band modeled is thermal noise. 
      //so, if there is no other current transmission, the TotalAvgPower will be 0, and the sinr will be infinity.
      // which can't be used in the ADR table.  
      // for now, if SINR = infinity, then don't count it. But should talk about this with Stephen.
      ...
      if (sinr_db > sinr_cutoff_db)
        {
          //maintain the avg SNR in the RxParams so it can be passed up to app layer for use in ADR.
          //CheckInterference is called at least once for each receiving packet, so we will have an average of at least two readings for each successful receive.

          loraWanRxParams->sinrAvg = 0.0; //this is the first receive of the packet, ensure counters are 0.
          loraWanRxParams->numSnrReadings = 0.0;

          if (!(isinf(sinr_db))) {
            loraWanRxParams->sinrAvg =  ((loraWanRxParams->sinrAvg * loraWanRxParams->numSnrReadings) + sinr_db) / (loraWanRxParams->numSnrReadings + 1); //maintain the average
            loraWanRxParams->numSnrReadings++;
            NS_LOG_INFO("sinr_db log first " << loraWanRxParams->sinrAvg);
          } else {
            if (loraWanRxParams->numSnrReadings == 0) {
              loraWanRxParams->sinrAvg = 0.0;
            } else {
        ...
        {
          //NS_LOG_INFO (this << "sinr_db < cutoff");
    ...
    {
      if(m_trxState == LORAWAN_PHY_RX_ON) {
        //NS_LOG_INFO (this << "in phy rx state");
      } else {
        //NS_LOG_INFO (this << "not phy rx state, in state: " << m_trxState);
      }

      if(!m_setTRXState.IsRunning ()) {
        //NS_LOG_INFO (this << "set trx state is not running");
      } else {
        //NS_LOG_INFO (this << "set trx state is running");
      }
      ...
      if (m_errorModel != 0)
        {
          ...
          *interferenceAndNoise += *m_noise;
          ...
          if (!(isinf(sinr_db))) {
            currentRxParams->sinrAvg =  ((currentRxParams->sinrAvg * currentRxParams->numSnrReadings) + sinr_db) / (currentRxParams->numSnrReadings + 1); //maintain the average
            NS_LOG_INFO("sinr_db log maintain " << currentRxParams->sinrAvg);
            currentRxParams->numSnrReadings++; 
          } else {
            NS_LOG_INFO (this << "sinr_db is infinity " << sinr_db);
            // TODO: log this?
          }
...
LoRaWANPhy::EndRx (Ptr<SpectrumSignalParameters> par)
{
  ...
  if (currentRxParams == params) 
    {
      NS_LOG_INFO("the sinr_avg value we pass up:" << params->sinrAvg);
      Ptr<Packet> currentPacket = currentRxParams->packet;
      ...
      if (!m_currentRxPacket.second.destroyed && !m_currentRxPacket.second.aborted)
        {
          ...
          if (!m_pdDataIndicationCallback.IsNull ())
            {
              m_pdDataIndicationCallback (currentPacket->GetSize (), currentPacket, 0, m_currentChannelIndex, params->dataRateIndex, params->codeRate, params->sinrAvg);
            }
        }
      else
        {
...
void 
LoRaWANPhy::UpdateNoiseFactorAndNoisePowerSpectralDensity(double noiseFactor)
{
  LoRaWANSpectrumValueHelper psdHelper;
  psdHelper.UpdateNoiseFactor(noiseFactor);
  const uint32_t freq = LoRaWAN::m_supportedChannels [0].m_fc; 
  m_noise = psdHelper.CreateNoisePowerSpectralDensity (freq); //note that this function has been modified to model noise across all channels.
}

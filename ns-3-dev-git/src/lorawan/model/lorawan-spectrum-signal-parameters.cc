  ...
  : SpectrumSignalParameters (p)
{
  ...
  numSnrReadings = p.numSnrReadings;
  sinrAvg = p.sinrAvg;
}
...
bool
LoRaWANSpectrumSignalParameters::operator==(const LoRaWANSpectrumSignalParameters& p)
{
  NS_LOG_INFO ("in herehere"); //TODO: is this still used???
  if ( (packet == p.packet) && (channelIndex == p.channelIndex) && (dataRateIndex == p.dataRateIndex) && (codeRate == p.codeRate) ) {
    NS_LOG_INFO ("operator == true!");
    return true;
  } else {
    NS_LOG_INFO ("operator == false!");
    return false;
  }

...
LoRaWANNetworkServer::HandleUSPacket (Ptr<LoRaWANGatewayApplication> lastGW, Address from, Ptr<Packet> packet)
{
  ...
  if (frmHdr.getFrameCounter () <= it->second.m_fCntUp && !firstRX) {
    ...
    it->second.m_recentRecv += 1; // total received packets since last DR change
    it->second.m_recentTotal += (frmHdr.getFrameCounter () - it->second.m_fCntUp); // total sent packets by the device since last DR change
	...
	LoRaWANAdrSnrRow newRow = {frmHdr.getFrameCounter(), phyParamsTag.GetSinrAvg (), 1}; //new packet, create a new row 
      ...
      double oldAvg = it->second.m_snrAvg;
      it->second.m_snrAvg = (it->second.m_snrAvg * it->second.m_snrCnt + phyParamsTag.GetSinrAvg ())  / (it->second.m_snrCnt + 1);
      it->second.m_snrCnt++;

      if(it->second.m_snrCnt == 1) {
        it->second.m_snrStdDev = 0; //stddev is 0 when we have 1 sample  
      } else {
        it->second.m_snrStdDev = std::sqrt(((it->second.m_snrCnt-1)*(it->second.m_snrStdDev) + (phyParamsTag.GetSinrAvg() - it->second.m_snrAvg)*(phyParamsTag.GetSinrAvg() - oldAvg)) / it->second.m_snrCnt); 
      }

      it->second.m_adrConfidenceCheck = AdrNetworkServerConfidence(key); //modifies m_adrConfidenceValue based on snrAvg, if above threshold thus chances m_adrConfidenceCheck
    } else {
      NS_LOG_INFO("LoRaWANPhyParamsTag not found on packet");
      NS_LOG_WARN (this << " LoRaWANPhyParamsTag not found on packet.");
    }
  ...
  if (packet->RemovePacketTag (phyParamsTag)) {
    if(it->second.m_lastDataRateIndex != phyParamsTag.GetDataRateIndex ())
    {
        it->second.m_recentRecv = 0;
        it->second.m_recentTotal = 0;  
    }
  ...
  if (frmHdr.getAdrAckReq ()) {
    ...
    it->second.m_EDSideDRPush = AdrEndDeviceConfidence(deviceAddr.Get ()); // function that decides whether to push DR down one peg
  }
...
LoRaWANNetworkServer::SendDSPacket (uint32_t deviceAddr, Ptr<LoRaWANGatewayApplication> gatewayPtr, bool RW1, bool RW2)
{
  ...
  if (it->second.m_downstreamQueue.size() > 0) {
    ...
    NS_LOG_INFO (this << " Sending downstream packet for dev addr " << deviceAddr);
    ...
    elementToSend.m_downstreamTransmissionsRemaining = element->m_downstreamTransmissionsRemaining;
      ...
      if(/*it->second.m_setAdr | */it->second.m_adrConfidenceCheck) { //If there is no data to be sent down, but the confidence function decides we need to send ADR data early, so schedule a new empty packet. TODO: make this a more general MAC command bool
        ///////////
        ...
        NS_LOG_INFO (this << " Generating empty downstream packet to send ADR for dev addr " << deviceAddr);
        ...
        NS_LOG_INFO (this << " Generating empty downstream packet to send Ack for dev addr " << deviceAddr);
  ...
  if(it->second.m_setAdr | it->second.m_adrConfidenceCheck) { //run ADR?
    /*
      TODO: 
    ...
    */
    ...
    //The SendDSPacket only gets called after a check with CanSendImmediatelyOnChannel is performed 
    //- so there will not be a situation where the ADR Algorithm is ran, but the result can't be sent because of the duty cycle limits, or because the device is not IDLE, or another TX state change is already scheduled.
    //- so if the NS reaches this state, then the ADR MAC command will be added to a device that will definitely be sent? TODO: double-check this.
    ...
    if(adrRes.status) {
      ...
      it->second.m_adrConfidenceCheck = false;
      it->second.m_adrConfidenceValue = 0;
      it->second.m_snrAvg = 0.0;
      it->second.m_snrCnt = 0.0;
    } else {
  ...
  } else if (it->second.m_EDSideDRPush) {
    LoRaWANADRAlgoritmResult edSideRes;
    if(it->second.m_lastTxPowerIndex == 0) {
      //try to improve situation by uping TX power to max
      edSideRes = {true, 15, 0, 0, 0, 0}; // i.e. {validADRrun, don't change DR, max TX pow, nbTrans and channel selection are not implemented}
    } else {
      //try to improve situation by decreasing the data rate
      uint8_t newDr = it->second.m_lastDataRateIndex - 1;
      edSideRes = {true, newDr, 15, 0, 0, 0}; // i.e. {validADRrun, DR-1, don't change TX pow, nbTrans and channel selection are not implemented}
    }
     fhdr.AddLoRaADRReq(edSideRes.dr, edSideRes.txPower, edSideRes.channelMask, edSideRes.chMaskCtrl, edSideRes.nbTrans);
     it->second.m_EDSideDRPush = false;
  }
...
bool
LoRaWANNetworkServer::AdrEndDeviceConfidence(uint32_t deviceAddr) 
{
    auto it = m_endDevices.find (deviceAddr);
  if (it == m_endDevices.end ()) { // end device not found
    NS_LOG_ERROR (this << " Could not find device info struct in m_endDevices for dev addr " << deviceAddr << ". Aborting ADR algorithm");
  ...
  //simple version: if PDR < 80%, return true
  float recv = (float) it->second.m_recentRecv;
  float total = (float) it->second.m_recentTotal; 
  
  if (total >= 10 && recv / total * 100 < 80.0) {
    return true;
  } else {
    return false;
  }
...
bool 
LoRaWANNetworkServer::AdrNetworkServerConfidence(uint32_t deviceAddr) 
{

  auto it = m_endDevices.find (deviceAddr);
  if (it == m_endDevices.end ()) { // end device not found
    NS_LOG_ERROR (this << " Could not find device info struct in m_endDevices for dev addr " << deviceAddr << ". Aborting AdrNetworkServerConfidence algorithm");
    return false;
  }

  if(it->second.m_lastDataRateIndex == 5 && it->second.m_lastTxPowerIndex == 7) 
  {
      NS_LOG_INFO("Running AdrNetworkServerConfidence, returning false for device " << deviceAddr << " as device already uses fastest DR and lowest TX power");
      return false;
  }

  //simple version: if at least 5 readings have been received, and expected change > 2, run algorithm early.
  if(it->second.m_snrCnt >= 5 && it->second.m_snrStdDev <= 2.5){
    double snrDr;
    if(m_snrCutoffValuesSource) {
      snrDr = m_adrSnrRequirementsSemtech[it->second.m_lastDataRateIndex].snr;
    } else {
    ...
    //double snrDr = m_adrSnrRequirements[it->second.m_lastDataRateIndex].snr;
    double SNRmargin = it->second.m_snrAvg - snrDr - it->second.m_marginDb;
    int nStep = int(SNRmargin/3);

    NS_LOG_INFO ("AdrNetworkServerConfidence Info: snrM: " << it->second.m_snrAvg << " snrDr: " << snrDr << " SNRmargin " << SNRmargin << " nStep " << nStep);

    if(nStep >= 1) {
      NS_LOG_INFO("Running AdrNetworkServerConfidence, returning true for device " << deviceAddr);
      return true;
    }
    NS_LOG_INFO("Running AdrNetworkServerConfidence, returning false for device " << deviceAddr << " as requested change not big enough");
    return false; 
  }
  NS_LOG_INFO("Running AdrNetworkServerConfidence, returning false for device " << deviceAddr << " as not enough readings");
  return false;
}

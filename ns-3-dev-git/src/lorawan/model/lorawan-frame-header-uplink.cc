...
LoRaWANFrameHeaderUplink::Serialize (Buffer::Iterator start) const
{
  ...
  if (m_serializeFramePort) { //if framePort is 0, then the MAC layer messages are read directly from the payload. Else they are read from here (from FOpts)
    ...
    if(m_macCommandsED[LinkADRAns].m_isBeingUsed) {
      i.WriteU8 (LinkADRAns); //write the command ID
      i.WriteU8 (m_status);
    }

    //TODO: add support for rest of MAC commands
  }
...
LoRaWANFrameHeaderUplink::Deserialize (Buffer::Iterator start)
{
  ...
  while(fOptsLen > 0) {
	...
	if(command_id == LinkADRAns) {  // LinkADRAns
      m_macCommandsED[LinkADRAns].m_isBeingUsed = true;
      m_status = i.ReadU8();
      fOptsLen--;
      nBytes += 1;

      // TODO: use this data in here (right now it is used outside of this class)
    }
    else {
      //TODO: err. 
    }
    //TODO: support the rest of the MAC commands
...
bool LoRaWANFrameHeaderUplink::AddLoRaADRAns (bool powerAck, bool drAck, bool channelMaskAck) {

  m_status = 0;

  uint8_t curr_len = m_frameControl & LORAWAN_FHDR_FOPTSLEN_MASK;

  if(curr_len + m_macCommandsED[LinkADRAns].m_size > LORAWAN_FHDR_FOPTSLEN_MAX_SIZE) {
  	return false;
  }

  //the operation will succeed

  m_frameControl += m_macCommandsED[LinkADRAns].m_size; //add to the FOptsLen, which is [3:0] of FCtrl
 
  uint8_t power = powerAck << 2;
  m_status |= power;

  uint8_t dr = drAck << 1;
  m_status |= dr;

  uint8_t channelMask = channelMaskAck;
  m_status |= channelMask; 

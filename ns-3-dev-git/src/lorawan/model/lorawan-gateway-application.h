...
/*
TODO: try to separate out gateway and network server again
*/
...
#define ADR_FREQUENCY 20 //the amount of uplink packets received from a device before the NS runs the NS-side ADR algorithm  
...
typedef struct LoRaWANAdrSnrRow {
  uint16_t frameCounter;
  double snrMax;
  uint8_t gtwDiversity; //currently not used.
...
typedef struct LoRaWANEndDeviceInfoNS {
  LoRaWANEndDeviceInfoNS () : m_deviceAddress(), m_rx1DROffset(0), m_lastDSGW(nullptr), m_lastGWs(), m_frameSNRHistory(), m_marginDb(5), m_setAdr(false), m_lastTxPowerIndex(0), //last 4 added by Joe
	...
	m_rw1Timer(), m_rw2Timer(), m_downstreamQueue(),m_downstreamTimer(), m_recentRecv(0), m_recentTotal(0), m_EDSideDRPush(false), m_adrConfidenceValue(0), m_adrConfidenceCheck(false),
  m_snrAvg(0), m_snrCnt(0), m_snrStdDev(0) {}
  ...
  /// ADR-related
  std::vector< LoRaWANAdrSnrRow> m_frameSNRHistory;  
  uint8_t         m_marginDb;
  bool            m_setAdr; 
  uint8_t         m_lastTxPowerIndex;
  ...
  uint32_t m_recentRecv;    // total number of received uplink packets since last DR change
  uint32_t m_recentTotal;   // total number of packets sent by the end device since last DR change (including missed packets, can be detected via the frame counter)
  bool     m_EDSideDRPush;  // makes a decision to push the DR of a link down by one if recent PDR has been poor. TODO: give this a better name
  uint32_t m_adrConfidenceValue;
  bool     m_adrConfidenceCheck;
  double   m_snrAvg;
  double   m_snrCnt;
  double   m_snrStdDev;
...
public:
  ...
  LoRaWANADRAlgoritmResult AdaptiveDataRate (uint32_t deviceAddr);

  bool AdrEndDeviceConfidence(uint32_t deviceAddr); 
  bool AdrNetworkServerConfidence(uint32_t deviceAddr);
...
private:
  ...
  static const std::vector<LoRaWANAdrSnrDrRequirement> m_adrSnrRequirementsSemtech;
  static const std::vector<LoRaWANAdrSnrDrRequirement> m_adrSnrRequirementsVDA;

  bool m_snrCutoffValuesSource; //true for Semtech doc, false for VdA. //TODO: better documentation on this.
};

 ...
 */
typedef Callback< void, uint32_t, Ptr<Packet>, uint8_t, uint8_t, uint8_t, uint8_t, double> PdDataIndicationCallback;
...
//    (Time time, LoRaWANPhyEnumeration oldState, LoRaWANPhyEnumeration newState);
  ...
  void UpdateNoiseFactorAndNoisePowerSpectralDensity(double noiseFactor);
...
private:
  ...
  TracedValue<uint32_t> m_bandwidthOfCurrentChannel; // in Hz

...
#define ADR_ACK_LIMIT 32 //should always be a power of 2
#define ADR_ACK_DELAY 32 //should always be a power of 2
...
private:
  ...
  bool            m_adr;        //if true, ADR mode is on
  bool            m_adrAckReq;
  bool            m_classB;
  bool            m_adr_NS;     //indication from the NS that it can handle it's side of the ADR. If false and m_adr is true, then a different algorithm is used.
  uint32_t        m_adrAckCnt;  //used by the ADR to count the number of frames sent since last receive 
  double          m_txPowerIndex;

  bool            m_doSendLinkAdrAns;
  bool            m_linkAdrAnsPowerAck;
  bool            m_linkAdrAnsDataRateAck;
  bool            m_linkAdrAnsChannelMaskAck;
